...
CREATE TABLE IF NOT EXISTS ctf_users (
    ...
    pending_device_info TEXT,
    login_request_status TEXT,
...
DECLARE
    ...
    v_is_active BOOLEAN;
BEGIN
    SELECT id, username, password_hash, role, active_session_id, last_active_at INTO found_user
    FROM ctf_users
    ...
    IF found_user.password_hash = crypt(p_password, found_user.password_hash) THEN
        -- Check if there is an active session (heartbeat within last 20 seconds)
        v_is_active := (found_user.active_session_id IS NOT NULL) AND 
                       (found_user.last_active_at IS NOT NULL) AND 
                       (found_user.last_active_at > NOW() - INTERVAL '20 seconds');
        
        -- If active and not forced, register a pending request and pause login
        IF v_is_active AND NOT p_force_login THEN
            v_session_id := gen_random_uuid();
            
            UPDATE ctf_users 
            SET pending_session_id = v_session_id,
                pending_device_info = p_device_info,
            ...
            RETURN json_build_object(
                ...
                'message', 'Awaiting active user authorization.'
            );
        END IF;

        -- Proceed directly if inactive or forced override
        v_session_id := gen_random_uuid();
        ...
        -- Return successful login payload
        RETURN json_build_object(
            'success', true, 
