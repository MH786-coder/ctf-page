...
export default function HackerEaglone() {
    ...
    useEffect(() => {
        const handleConsoleToggle = (e: Event) => {
            const customEvent = e as CustomEvent;
            if (customEvent.detail) {
                setIsConsoleOpen(!!customEvent.detail.isOpen);
            }
        };

        window.addEventListener('ctf_console_toggle', handleConsoleToggle);
        return () => {
            window.removeEventListener('ctf_console_toggle', handleConsoleToggle);
        };
    }, []);
    ...
    const greet = () => {
        const username = sessionStorage.getItem('ctf_username') || 'operative';
        const welcomeText = `Welcome back, ${username}. I am hacker eaglone, ready to decrypt target arrays.`;
        speak(welcomeText);
        setCurrentTip(`WELCOME: Operative ${username}. Active session secured.`);
    };

    useEffect(() => {
        const handleSuccess = (e: Event) => {
            const customEvent = e as CustomEvent;
            const count = customEvent.detail?.completedCount || 0;
            
            let voiceText = "";
            let bubbleText = "";
            
            if (count <= 1) {
                voiceText = "Awesome! That is your first captured flag. You are officially in the matrix.";
                bubbleText = "COMPROMISED! First node captured. System access is growing.";
            } else if (count <= 3) {
                voiceText = `Outstanding! That is ${count} flags captured. You are making rapid progress.`;
                bubbleText = `DECRYPTED! ${count} targets compromised. Keep scaling the firewall.`;
            } else if (count <= 5) {
                voiceText = `Spectacular! ${count} flags secured. You are starting to play like a professional hacker.`;
                bubbleText = `SECURED! ${count} nodes offline. Firewall threat level: critical.`;
            } else {
                voiceText = `Incredible! ${count} challenges solved! You are a legendary operative now. The network is yours!`;
                bubbleText = `GRID_DOMINATOR! ${count} flags captured. Elite rank achieved.`;
            }
            
            speak(voiceText);
            setCurrentTip(bubbleText);
            setShowBubble(true);
        };

        const handleFailure = () => {
            const wrongMessagesVoice = [
                "Access denied. Don't worry, operative, double check the encryption key and try again.",
                "Invalid payload hash. Standard protocol is to analyze the hint and recalculate.",
                "Decryption failed. A minor setback, keep scanning the headers. You will crack it.",
                "Connection rejected. Remember, failure is just a debugging step to success."
            ];
            const wrongMessagesBubble = [
                "ACCESS_DENIED // Recalibrate flag string and retry.",
                "DECRYPTION_ERROR // Tip: Read hints for pattern markers.",
                "PAYLOAD_REJECTED // Keep digging. You got this, operative.",
                "LINK_LOST // Check syntax and run decrypt sequence again."
            ];
            
            const randomIdx = Math.floor(Math.random() * wrongMessagesVoice.length);
            speak(wrongMessagesVoice[randomIdx]);
            setCurrentTip(wrongMessagesBubble[randomIdx]);
            setShowBubble(true);
        };

        window.addEventListener('ctf_flag_success', handleSuccess);
        window.addEventListener('ctf_flag_failure', handleFailure);

        return () => {
            window.removeEventListener('ctf_flag_success', handleSuccess);
            window.removeEventListener('ctf_flag_failure', handleFailure);
        };
    }, []);
    ...
    return (
        <div 
            className="fixed left-6 z-[200] flex items-end gap-4 pointer-events-none transition-all duration-500 ease-in-out"
            style={{ bottom: isConsoleOpen ? '470px' : '24px' }}
        >
